...
| `AV-APP-01` | 外部 `observe` / `hook` / `act` application loop | `supported` | public `AgentViewApp` 提供 epoch fencing、full/partial view、stale-turn rejection 和 wake subscription。mounted external path 让作者通过 `ExternalReply<Contract>` 绑定 grammar/pure decoder，并区分带 token 的 Actionable Prompt 与无 token 的 Passive Presentation；User publication/ack/CAS 仍属于 advanced host。 |
