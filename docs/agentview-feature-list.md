...
stream/tool/effect 已有可执行局部实现。Forgotten City 现在有两个 opt-in、SQLite-backed
Chess session facade：provider-backed AgentLoop 与 external CLI。External 已证明跨进程
`System attach/ack -> full Actionable ack -> Passive -> delta -> User-only full resync -> delta`，
且每个 action 都绑定 exact serializable handle；AgentLoop 已证明 recording OpenAI System-once、
replacement-owner structural delta、幂等 player Commit 和 durable Stockfish job。
它们仍不是完整 production migration：常驻恢复 supervisor、server 安装、真实远端 provider、
显式 transport handoff 和统一 retained component definition 尚未完成。
...
| `AV-F03` Semantic View Update | `implemented` | Public `DiffSlot`、多种集合 diff、full/delta/delete、`UserDocumentCursor` 和失败回滚；SQLite external Chess 与 repository skill 已实跑 `full Actionable -> full Passive -> delta Actionable`，证明 consumer delivery ack、base receipt 和 passive non-advancement。consumer 丢失 baseline 时会 tombstone 当前未确认 delta，只 full-resync 下一张 User，不重发 System；确认该 full 后恢复 delta。provider Chess 另证明严格 semantic gate 与 publication 成功后才推进 baseline：首轮 semantic reject 后仍为 full；已有成功 baseline 后 reject，下一轮仍从最后成功 baseline 发 delta；schema-v3 durable mounted owner 跨 factory reopen 会恢复并继续该 baseline，schema-v2 则安全 full-resync 后迁移。 | replacement logical-consumer/transport handoff policy，以及真实 remote provider 丢失 baseline 后的 resync trace。 |
...
| `AV-F09` Reactive Observe/Act | `partial` | Public compatibility `AgentViewApp` 仍支持 `observe/hook/act_with_sink`。advanced external boundary 将纯 `ExternalPromptComponent`、shared typed `ExternalReply<Contract>` 与 host-owned `MountedExternalController` 分开。System 只进入 `complete_epoch` 一次；Actionable Prompt 与 Passive Presentation 是不同 frame，后者没有 action token。User bytes 只通过 publish CAS 的 immutable candidate 进入 host outbox，public frame 仅返回 receipt；controller 的 `act` 必须等待 consumer ack。Schema v5 持久化 `AwaitingDelivery`、acknowledged cursor 和 full-resync fence。Forgotten City 的 concrete SQLite facade 进一步固定一个 durable logical consumer，要求先完成 System attach/ack，再以 serializable exact action handle 显式 ack User 并提交 raw XML；多进程 `observe/ack/act/hook/resync`、真实 Stockfish、receipt/reply replay、consumer conflict、passive non-advancement 和 User-only resync 均有证明。 | bounded receipt retention、server/supervisor 安装、replacement-consumer/transport handoff 和 process-kill fault matrix。 |
...
| AgentView mounted chess authoring | `cargo run --example chess_engine_mounted` | 通过；独立 consumer review 将主路径收敛为一个 `chess_agent` component、一次 `mount` 和两次 `run_turn`。该 prompt-only component 通过 `try_prompt_component` 复用既有 fallible chess User-document builder，与 Hello World 保持同一作者形状。同一 chess POM 在 `e2e4` 前后捕获两份 board snapshot，mounted trace 只 attach 一份 786-byte System，生成 5124/4100-byte User。完整 `observe/act/hook` 已由 mounted external CLI 证明；本例仍刻意只教 prompt authoring。 |
...
| AgentView mounted external Chess | `cargo test --example chess_engine_mounted_external --quiet`、`cargo test --test agentview_cli --quiet` | 9/9 与 5/5 通过；该教学 daemon 仍为 in-memory，生产形状的 SQLite 证据由下一行 Forgotten City facade 提供。 |
| Forgotten City durable Chess facades | `cargo test -p engine player::mounted_chess -- --nocapture`、`cargo run -p engine --example chess_external -- <command>` | 20 passed、1 ignored；AgentLoop 与 external facade 都隐藏 AgentView persistence types，并以 durable transport claim 防止同 session 混用。External 测试及真实跨进程 Stockfish trace 覆盖 System attach/ack once、logical consumer conflict、exact action handle/raw XML、full/Passive/delta、receipt/reply replay、active-delta tombstone、User-only full resync、显式确认 full 后恢复 delta、wake 边界与 SQLite reopen。`chess_agentloop` 另提供真实 OpenAI 配置入口；本次未提供远端凭据。 |
...
3. 收敛 AgentLoop `chess_feature` 与 external `external_prompt_component` 为一个 retained
   component source，再用 Forgotten City Player SelectIntent 验证 author、provider、
...
| Forgotten City agents | compatibility AgentLoop 的本地编译和行为测试通过；SelectIntent 有 consumer-shaped mounted component/Live/runtime/provider proof；Chess 已有 SQLite-backed AgentLoop 与 external session facade、幂等 domain、durable Stockfish、跨进程 CLI、User-only resync 和 reopen structural-delta 纵向证明。 | 尚不支持 production mounted Player/NPC/GM；没有真实远端 provider、常驻 worker supervisor、server 安装、显式 transport handoff、durable id-keyed cancel recovery/control 或完整 golden trace。 |
